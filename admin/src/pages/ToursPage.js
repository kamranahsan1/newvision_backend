...
export default function ToursPage() {
  ...
  const LoadCategories = async () => {
    const response = await axios.get(`${API_URL}/generate-tourv2?resultPerPage=100`);
  ...
  const HandleDelete = async (id) => {
    await axios.delete(`${API_URL}/DeleteTours/${id}`);
    LoadCategories();
  };
                ...
                {Data.length > 0 ? (
                  <TableBody>
                    {Data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                      const { name, description, mainImage, Day, time, _id } = row;
