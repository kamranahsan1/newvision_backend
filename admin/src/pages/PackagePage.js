...
const TABLE_HEAD = [
  ...
  { id: 'countryCode', label: 'Code', alignRight: false },
...
export default function PackagePage() {
  ...
  const LoadCategories = async () => {
    const response = await axios.get(`${API_URL}/packages?resultPerPage=100`);
    console.log(response.data.packages);
    SetData(applySortFilter(response.data.packages, getComparator(order, orderBy), filterName));
    setPage(response.data.packagesCount);
    SetAllData(applySortFilter(response.data.packages, getComparator(order, orderBy), filterName));
  ...
  const HandleDelete = async (id) => {
    await axios.delete(`${API_URL}/DeletePackage/${id}`);
    LoadCategories();
  };
    ...
    <>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button variant="contained" startIcon={<Iconify icon="eva:plus-fill" />} onClick={NavigateToPackage}>
            New Package
          </Button>
        </Stack>
                ...
                {Data.length > 0 ? (
                  <TableBody>
                    {Data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                      const { name, mainImage, country, countryCode, _id, description } = row;
