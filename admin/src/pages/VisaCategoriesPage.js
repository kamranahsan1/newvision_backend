...
import { API_URL, setImage, setBaseUrlImage } from '../constants/General';
// components
...
const TABLE_HEAD = [
  ...
  { id: 'Actionss', label: 'Actions', alignRight: false },
  // { id: '' },
];
...
export default function VisaPage() {
  ...
  const LoadCategories = async () => {
    const response = await axios.get(`${API_URL}/viewcategories?resultPerPage=100`);
  ...
  const HandleDelete = async (id) => {
    await axios.delete(`${API_URL}/DeleteVisaCategory/${id}`);
    LoadCategories();
  };
  ...
  return (
    <>
      <Helmet>
        <title> Visa Category | New Vision </title>
      </Helmet>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Button variant="contained" startIcon={<Iconify icon="eva:plus-fill" />} onClick={NavigateToparent}>
            New Visa Category
          </Button>
        </Stack>
                ...
                {Data.length > 0 ? (
                  <TableBody>
                    {Data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                      const { name, description, mainImage, parent, slug, _id, status } = row;
